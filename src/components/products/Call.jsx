...
const Call = () => {
  ...
  return (
    <div className="modal-box bg-slate-900 w-11/12 mx-auto shadow-orange-400 shadow-md">
      ...
      <p className="py-4 call-modal-title text-center">
        Buy and Service a product by phone call and whatsapp
      </p>
      <div className="modal-action">
        <form method="dialog">
          <button className=" call-modal-close ">x</button>
        </form>
        <div className="w-full grid grid-cols-2 mx-auto">
          <button
            className="btn w-10/12 text-center pop ml-5"
            onClick={handleCall}
          >
            <FaPhoneVolume size="13" className="mx-1" /> Call
          </button>
          <button className="btn w-10/12 text-center pop" onClick={handleClick}>
